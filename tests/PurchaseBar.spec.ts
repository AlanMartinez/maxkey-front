...
describe('PurchaseBar', () => {
  it('shows the product name with its prices and emits both actions', async () => {
    const wrapper = await mountSuspended(PurchaseBar, { props: { productName, variant } })
  ...
  it('lets the buyer switch variants from the bar and marks the selected one', async () => {
    ...
    const wrapper = await mountSuspended(PurchaseBar, { props: { productName, variants: [other, variant], modelValue: 'v2', recommendedId: 'v2', variant } })
  ...
  it('hides the chip row with a single variant', async () => {
    const wrapper = await mountSuspended(PurchaseBar, { props: { productName, variants: [variant], modelValue: 'v2', variant } })
    expect(wrapper.find('[role="group"]').exists()).toBe(false)
  })

  it('disables the actions without a variant and reflects the added state and errors', async () => {
    const wrapper = await mountSuspended(PurchaseBar, { props: { productName, variant: undefined } })
  ...
  it('shows an orange offer badge above 50%', async () => {
    const wrapper = await mountSuspended(PurchaseBar, { props: { productName, variant: { ...variant, price: 4900, oldPrice: 10000 } } })
